...
/*   Updated: 2023/02/22 15:21:10 by vbenneko      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
t_ray	translate_ray(t_ray ray, t_cylinder cylinder)
{
...
*/
t_ray	rotate_ray(t_ray ray, t_cylinder cylinder000)
{
...
t_ray	adjust_ray(t_ray ray, t_cylinder cylinder)
{
	ray = translate_ray(ray, cylinder);
	ray = rotate_ray(ray, cylinder);
	return (ray);
}

// static float	get_cylinder_distance(t_ray ray, t_cylinder cylinder)
// {
// 	const t_ray	adjusted_ray = adjust_ray(ray, cylinder);
	...
	t_ray ray, t_object *object, t_data *data)
{
	...
	(void)data;
	info.distance = get_cylinder_distance(ray, cylinder);
	...
	info.surface_normal = rt_normalized(rt_sub(rt_get_ray_point(ray,
					info.distance), cylinder.origin)); // TODO: CHANGE TO WORK FOR CYLINDER NO SPHERE
