					...
					char *string, bool draw)
{
	...
	if (draw)
	{
		images_ptr[data->debug_image_index] = mlx_put_string(data->mlx, string,
				0, FONT_HEIGHT * (int)data->debug_image_index);
		if (images_ptr[data->debug_image_index] == NULL)
			return (rt_print_error(ERROR_MLX));
		instance = images_ptr[data->debug_image_index]->instances;
		mlx_set_instance_depth(instance, DEBUG_DRAWING_MLX_DEPTH);
	}
...
t_status	rt_draw_debug_lines(t_data *data)
{
	...
	rt_draw_debug_nbr_line(data, images,
		(int)(1 / data->mlx->delta_time), " frames/s");
	rt_draw_debug_nbr_line(data, images,
		(int)(1000 * data->mlx->delta_time), " ms/frame");
	rt_draw_debug_nbr_line(data, images,
		(int)ft_get_allocation_count(), " allocations");
	rt_draw_debug_nbr_line(data, images,
		(int)ft_get_bytes_allocated(), " bytes allocated");
	rt_draw_debug_nbr_line(data, images,
